...
{
    private const string MutexName = "DevToy_SingleInstance_Mutex";
    internal const string PipeName = "DevToy_Pipe";
    ...
    private static void RunInstalledInstance()
    {
        ...
        if (!isNew)
        {
            ...
            SendToPipe("DevToy", "DevToy is ready.", NotificationType.Info);
            return;
        }

        // Load last history entry or show a default welcome
        var latest = ResponseHistory.GetLatest();
        string title = latest?.Title ?? "DevToy";
